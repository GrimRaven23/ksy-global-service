...
export default function Home() {
  ...
  const quickActions = [
    ...(can("proforma.create") ? [{ label: "Pro Forma", desc: "Facture pro forma", icon: FileText, href: "/proforma", color: "bg-navy/5 text-navy dark:bg-navy/10 dark:text-white border-navy/10 hover:border-navy/25" }] : []),
    ...(can("proforma.create") ? [{ label: "Définitive", desc: "Facture définitive", icon: Receipt, href: "/definitive", color: "bg-blue-bg text-blue dark:bg-blue/10 dark:text-white border-blue/15 hover:border-blue/30" }] : []),
    ...(can("delivery.create") ? [{ label: "Bon de Livraison", desc: "Bon de livraison", icon: Truck, href: "/bl", color: "bg-gold-bg text-gold dark:bg-gold/10 dark:text-white border-gold/15 hover:border-gold/30" }] : []),
  ];
    ...
    <AppShell>
      {loading ? (
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-5 lg:px-6 py-4 sm:py-6 space-y-5 sm:space-y-6">
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
            {[
              { label: "Documents", value: stats.totalDocuments, icon: FileText, color: "text-navy dark:text-white", bg: "bg-navy/5 dark:bg-navy/10" },
              { label: "Revenus (FCFA)", value: fmtNum(stats.totalRevenue), icon: BarChart3, color: "text-green dark:text-green", bg: "bg-green-bg dark:bg-green/10" },
              { label: "Ce mois", value: stats.documentsThisMonth, icon: Clock, color: "text-blue dark:text-blue", bg: "bg-blue-bg dark:bg-blue/10" },
              { label: "Livraisons", value: stats.totalDeliveryNotes, icon: Truck, color: "text-gold dark:text-gold", bg: "bg-gold-bg dark:bg-gold/10" },
            ].map((s) => (
                ...
                {quickActions.map((a) => (
                  ...
                  >
                    <div className="flex items-start justify-between mb-3">
                      <div className="w-10 h-10 sm:w-11 sm:h-11 rounded-xl bg-navy/5 dark:bg-white/5 flex items-center justify-center group-hover:scale-110 transition-transform">
                        <a.icon className="w-5 h-5 text-navy dark:text-white" />
                      </div>
                      <ArrowRight className="w-4 h-4 text-txt3 dark:text-white/40 group-hover:text-navy dark:group-hover:text-white group-hover:translate-x-0.5 transition-all" />
                    </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
                      ...
                      <table className="w-full min-w-[500px]">
                        <thead>
                          <tr className="border-b border-bdr/60">
                            <th scope="col" className="text-left text-[10px] sm:text-[11px] font-semibold text-txt3 dark:text-white/50 uppercase tracking-wide pb-2.5">Num</th>
                            <th scope="col" className="text-left text-[10px] sm:text-[11px] font-semibold text-txt3 dark:text-white/50 uppercase tracking-wide pb-2.5">Type</th>
                            <th scope="col" className="text-left text-[10px] sm:text-[11px] font-semibold text-txt3 dark:text-white/50 uppercase tracking-wide pb-2.5 hidden md:table-cell">Client</th>
                            <th scope="col" className="text-right text-[10px] sm:text-[11px] font-semibold text-txt3 dark:text-white/50 uppercase tracking-wide pb-2.5">Total</th>
                            <th scope="col" className="text-left text-[10px] sm:text-[11px] font-semibold text-txt3 dark:text-white/50 uppercase tracking-wide pb-2.5">Statut</th>
                          </tr>
                        </thead>
                        <tbody>
                          {recentDocs.map((d) => (
                            ...
                            >
                              <td className="py-2.5 text-xs sm:text-sm font-semibold text-navy dark:text-white">{d.num}</td>
                              <td className="py-2.5"><Badge color={typeColor(d.type)}>{typeLabel(d.type)}</Badge></td>
                              <td className="py-2.5 text-xs text-txt2 dark:text-white/60 hidden md:table-cell">{d.customerName || "—"}</td>
                      ...
                      {recentDocs.map((d) => (
                        ...
                        >
                          <div className="flex items-start justify-between gap-2 mb-1.5">
                            <span className="text-xs font-bold text-navy dark:text-white">{d.num}</span>
                          ...
                          <div className="flex items-center gap-1.5 flex-wrap">
                            ...
                            {d.customerName && <span className="text-[10px] text-txt3 dark:text-white/40 truncate ml-1">{d.customerName}</span>}
                          </div>
                        </button>
                      ))}
                    </div>
            ...
            <Card>
              ...
              {activity.length === 0 ? (
                <p className="text-xs text-txt3 dark:text-white/40 text-center py-8">Aucune activité</p>
              ) : (
                <div className="space-y-3">
                  {activity.map((e) => (
                    <div key={e.id} className="flex items-start gap-2.5">
                      <div className="w-1.5 h-1.5 rounded-full bg-navy/20 dark:bg-white/30 mt-1.5 shrink-0" />
                      <div className="min-w-0">
                        <p className="text-[11px] sm:text-xs text-txt dark:text-white/80 truncate">{e.action.replace(/_/g, " ").toLowerCase()}</p>
                        <p className="text-[10px] text-txt3 dark:text-white/40">{relativeTime(e.createdAt)}{e.user ? ` • ${e.user.name}` : ""}</p>
                      </div>
                    </div>
                  ))}
                </div>
